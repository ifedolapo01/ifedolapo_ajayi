...
- The three main projects: the remaining `build` lines, how stories are
  submitted on Beyond the Pain, what lives in the Gidiammini store, which
  screens you built on Gidiamcare and the current status of it.
...
`DemoPanel` renders **twice** inside `Xray`, once as built and once as drawn.
